...
def get_conn():
    user, password, host, port, db = parse_db_url(DB_URL)
...
def guardar_props(props):
    ...
    for p in props:
        try:
            url = p.get("url","") or f"{p.get('fuente','')}_{p.get('titulo','')}_{guardadas}"
            cur.execute("""
    ...
    cur.close()
    conn.close()
    print(f"💾 {guardadas} props guardadas")
...
@app.route("/")
def home():
    return jsonify({"status": "RodiProp API OK", "version": "5.0", "total": contar_props()})
