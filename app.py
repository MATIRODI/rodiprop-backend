            ...
            try:
                ...
                for card in cards:
                    try:
                        ...
                        # Extraer ubicación del título o URL si no hay campo específico
                        ubicacion_raw = u.text.strip() if u else ""
                        if not ubicacion_raw and href:
                            # Intentar extraer barrio de la URL
                            partes = href.split("/")
                            if len(partes) > 3:
...
def chequear_alertas():
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("""SELECT id,nombre,email,whatsapp,zona,tipo,operacion,
            ...
            AND (plan='premium' OR COALESCE(alertas_enviadas_count,0) < 7)""")
        ...
        for u in usuarios:
            try:
                ...
                if u.get("ambientes"):
                    ...
                    params += [f"%{u['ambientes']} amb%", f"%{u['ambientes']} dorm%"]
                ...
                for prop in cur2.fetchall():
                    ...
                    if enviar_whatsapp(u["whatsapp"], mensaje):
                        ...
                        cur2.execute("UPDATE usuarios SET alertas_enviadas_count = COALESCE(alertas_enviadas_count,0) + 1 WHERE id=%s", (u["id"],))
                        conn2.commit()
                cur2.close()
                conn2.close()
...
def registro():
    ...
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("""INSERT INTO usuarios (nombre,email,whatsapp,zona,tipo,operacion,precio_min,precio_max,ambientes,cocheras)
...
def fix_moneda():
    """Corregir moneda ARS a USD para LaVoz"""
    try:
        conn = get_conn()
        cur = conn.cursor()
